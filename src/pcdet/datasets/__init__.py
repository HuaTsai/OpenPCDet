...
# Each dataset implementation pulls in its own devkit (av2, lyft-sdk,
# waymo-open-dataset, pandaset, ...). Import each optionally so a minimal env
# (e.g. a nuScenes-only setup) can use pcdet without installing every devkit;
# an unavailable dataset is simply not registered.
__all__ = {'DatasetTemplate': DatasetTemplate}

for _name, _module, _cls in [
    ('KittiDataset', '.kitti.kitti_dataset', 'KittiDataset'),
    ...
    ('CustomDataset', '.custom.custom_dataset', 'CustomDataset'),
]:
    try:
        from importlib import import_module
        __all__[_name] = getattr(import_module(_module, __name__), _cls)
    except ImportError:
        pass
