...
def _shm_path(name):
    """Map a 'shm://<key>' name to its backing file under /dev/shm (tmpfs)."""
    key = name[len('shm://'):] if name.startswith('shm://') else name
    return os.path.join('/dev/shm', key)


def sa_create(name, var):
    # Backed by a numpy memmap on /dev/shm (RAM-backed tmpfs). The .npy header
    # stores shape/dtype, so sa_attach() can rebuild the array from the name
    # alone in any process. Replaces the unmaintained SharedArray package.
    x = npformat.open_memmap(_shm_path(name), mode='w+', dtype=var.dtype, shape=var.shape)
    x[...] = var[...]
    x.flush()
    return x
...
def sa_attach(name):
    return np.load(_shm_path(name), mmap_mode='r')
